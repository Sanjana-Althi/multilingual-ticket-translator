...
def run_detection_and_translation(ticket_text: str, ticket_id: str = "TICKET") -> dict:
    ...
    if language_name not in SUPPORTED_LANGUAGES:
        result["error"] = f"Unsupported language: {language_name}"; return result
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "single_res" in st.session_state:
        ...
        with col_a:
            st.markdown(f'<div class="info-card"><div class="info-label">Original Ticket</div><div class="info-value">{res["original_ticket"]}</div></div>', unsafe_allow_html=True)
        with col_b:
            st.markdown(f'<div class="info-card"><div class="info-label">English Translation</div><div class="info-value">{res["english_ticket"]}</div></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="lang-badge">🌍 {res["original_language"]} &nbsp;·&nbsp; {res["language_code"]}</div>', unsafe_allow_html=True)
        st.markdown("<hr class='divider'>", unsafe_allow_html=True)
        ...
        if st.button("🔁 Translate Reply", key="translate_reply_btn"):
            ...
            with st.spinner("Translating reply back to customer language…"):
                try:
                    ...
                    glossary_terms = []
                translated, err = translate_engineer_reply(reply_to_use, res["original_language"], glossary_terms)
            if err:
                ...
                try:
                    saved = save_ticket_record(
                        ...
                        english_ticket=res["english_ticket"], engineer_reply=reply_to_use,
                        translated_reply=translated, ticket_id=res["ticket_id"],
                    )
    ...
    if "single_translated" in st.session_state:
        ...
        with col_d:
            st.markdown(f'<div class="info-card"><div class="info-label">Translated Reply ({res["original_language"]})</div><div class="info-value">{translated}</div></div>', unsafe_allow_html=True)
        ...
        for idx, res in enumerate(processed):
            ...
            with st.expander(f"{icon}  Ticket {ticket_num}  ·  {fname}", expanded=True):
                if has_error:
                    ...
                    with col1:
                        st.markdown(f'<div class="info-card"><div class="info-label">File Name</div><div class="info-value">{fname}</div></div><div class="info-card"><div class="info-label">Original Ticket</div><div class="info-value">{res["original_ticket"]}</div></div>', unsafe_allow_html=True)
                    with col2:
                        st.markdown(f'<div class="info-card"><div class="info-label">Detected Language</div><div class="info-value">{res["original_language"]} ({res["language_code"]})</div></div><div class="info-card"><div class="info-label">English Translation</div><div class="info-value">{res["english_ticket"]}</div></div>', unsafe_allow_html=True)
